...
.h1-actions { margin-left: auto; display: flex; gap: 8px; align-items: center; }
...
fetch('/api/inventory').then(r => r.ok ? r.json() : Promise.reject()).then(() => {
  ...
  // Page-level actions, top right in the title row. The framework page
  // deploys freely — public content, standing URL. A map repository's page
  // is local by default (#34): deploy exists, but the server answers with
  // the ask question and the confirmation consents to that build only. Its
  // target is the repo's OWN private artifact, never the framework's URL —
  // and that artifact is never shared; the button copy says so.
  fetch('/api/meta').then(r => r.json()).then(meta => {
    const actions = document.createElement('span');
    actions.className = 'h1-actions';
    document.querySelector('h1').appendChild(actions);
    // #36: the artifact chip — is a snapshot published, and is it current?
    const art = meta.artifact || {};
    const chip = document.createElement('span');
    const when = art.published_at ? ' (' + art.published_at + ')' : '';
    if (!art.url) {
      chip.className = 'tag';
      chip.textContent = 'artifact: none';
      chip.title = 'No published snapshot. The deploy button builds one.';
    } else if (art.current === true) {
      chip.className = 'tag trust-verified';
      chip.textContent = 'artifact: current' + when;
      chip.title = 'The published snapshot matches this page: ' + art.url;
    } else if (art.current === false) {
      chip.className = 'tag effect-mutating';
      chip.textContent = 'artifact: stale' + when;
      chip.title = 'The published snapshot no longer matches this page — '
                 + 'deploy to refresh it: ' + art.url;
    } else {
      chip.className = 'tag';
      chip.textContent = 'artifact: published' + when;
      chip.title = 'Currency unknown — no publish hash recorded yet: ' + art.url;
    }
    actions.appendChild(chip);
    if (meta.share_url) {
      const s = document.createElement('button');
      const stale = art.current === false;
      s.className = 'run share' + (stale ? ' stale' : '');
      s.textContent = meta.framework ? 'share link' : 'private link';
      s.title = !meta.framework
        ? 'Copy this map repository\\'s PRIVATE artifact URL — your account '
          + 'only, never to be shared: ' + meta.share_url
        : meta.deployed
      ...
      s.addEventListener('click', () => copy(meta.share_url).then(() => {
        ...
        setTimeout(() => { s.textContent = was; }, 1500);
      }));
      actions.appendChild(s);
    }
    const d = document.createElement('button');
    d.className = 'run deploy';
    d.textContent = meta.framework ? 'deploy' : 'deploy (private)';
    d.title = !meta.framework
      ? 'Build this page for its own private Claude artifact — asks first; '
        + 'nothing leaves this machine until an agent session publishes'
      : meta.deployed
    ...
    d.addEventListener('click', () => {
      // After a build the click copies the handoff command — it must never
      // loop back into consent + rebuild; that reads as a broken button.
      if (d.dataset.built === '1') {
        copy(d.dataset.cmd).then(() => {
          const was = d.textContent;
          d.textContent = 'copied ✓ — run it in Claude Code';
          setTimeout(() => { d.textContent = was; }, 1800);
        });
        return;
      }
      postDeploy(d, []);
...
async function postDeploy(d, consent) {
  ...
  try {
    ...
    if (r.status === 409 && data.ask) {
      // Same ask channel as a mutating run: the server states the question,
      // a human answers it, consent applies to THIS build only (#34).
      d.textContent = label;
      if (confirm(data.question)) return postDeploy(d, consent.concat([data.ask]));
      d.textContent = 'not built: consent declined';
      setTimeout(() => { d.textContent = label; }, 2500);
      return;
    }
    if (data.command) {
      // Terminal state: the server can only build — publishing needs an
      // interactive agent session. Say so in the page, not a tooltip, and
      // turn the button into a copy of the handoff command.
      d.dataset.built = '1';
      d.dataset.cmd = data.command;
      d.textContent = 'build ready — copy ' + data.command;
      d.title = 'Click to copy. ' + data.note;
      let note = document.querySelector('.deploy-note');
      if (!note) {
        note = document.createElement('p');
        note.className = 'sub deploy-note';
...
def build_stamp():
    """Git identity of a deploy build (#36): what the snapshot was made from."""
    def git(*args):
        try:
    ...
    return {"commit": git("rev-parse", "--short", "HEAD") or "?",
            "dirty": bool(git("status", "--porcelain", "--", "sites", "projects",
                              "schema", "scripts", "docs", "config.yaml")),
            "built_at": datetime.now().strftime("%Y-%m-%d %H:%M")}


def content_hash(html_text):
    """Fingerprint of the deploy form of the page. Computed BEFORE the stamp
    is inserted, so rebuilding identical content stays 'current' (#36)."""
    return "sha256:" + hashlib.sha256(html_text.encode("utf-8")).hexdigest()


def deploy_build(out_path):
    """Write the deploy build: the github-links fragment page, stamped in the
    footer with what it is a snapshot of, plus build-info.json beside it —
    the hash the publish step records for the staleness chip (#36)."""
    html_text = render(base=github_base(), fragment=True)
    info = build_stamp()
    info["hash"] = content_hash(html_text)
    stamp = (" Snapshot of " + info["built_at"] + ", commit " + info["commit"]
             + (" + uncommitted changes" if info["dirty"] else "") + ".")
    html_text = html_text.replace("</footer>", stamp + "</footer>", 1)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write(html_text)
    with open(os.path.join(os.path.dirname(out_path), "build-info.json"),
...
def render(base=LOCAL, fragment=False):
    ...
    # A deployment's page is named after its repository, not after the tool
    # that rendered it — only the framework's own tree is titled "SiteMapper".
    here = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    title = "SiteMapper" if ROOT == here else os.path.basename(ROOT)
    ...
    a("<footer>Generated by <code>scripts/inventory.py</code> from the YAML in "
      "this repository — do not edit. "
