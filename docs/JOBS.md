...
| Deploy the dashboard | **automatic on push** (cloud routine `deploy-dashboard`, + daily 03:00 UTC safety net); manual fallback: the live page's deploy button (builds) + `/deploy-dashboard` (publishes) — **to the standing URL** <https://claude.ai/code/artifact/88589587-c88d-4092-b4d7-3e1ac07e47a7>. Framework page only at this URL; a map repository's page deploys — consent-gated, manual only — to its OWN private artifact, registered in its config.yaml (#34, PERMISSIONS.md "Dashboard deploy") | shareable snapshot of THIS repo's page | cloud routine / skill ([deploy-dashboard.md](skills/deploy-dashboard.md)) |
